public class sortieren{
		...
		public static void zahlenSort(int[] z){							//Methode zum sortieren der Zahlenreihe
						int n = z.length;								//der wert n enthält die Länge der Zahlenliste und ist nun unter n und auch unter z.lenght bekannt, es ist sowohl auch die Länge der Array bekannt
						for(int i = 0; i< n; i++){						//starten bei 0, weiter solange i kleiner als die Länge der Zahlenliste ist
						int min = z[i];									//der Wert min nimmt den kleinsten Wert an 
						int minIndex = i;								//der Wert minIndex ist nun i ( gleich 0)
								for(int j = i+1; j< n; j++){		//START 1 SCHLEIFE. Starten bei i+1 also bei 1, weiter solange j kleiner als die Länge der Zahlenliste ist 
									if (min > z[j]){					//wen min kleiner als der Wert j ist dann***
									min = z[j];							//**dann ist das minimum der neue Wert für j
		...
		for(int i=0;i<n;i++){									//starten bei 0, weiter solange i kleiner als die Länge der Zahlenliste ist
		s=s+z[i]+",";  													//füge hinter jedes Element ein , hinzu
		}																// ENDE
		...
		s=s+"\n";  														//füge der Zeichenkette eine neue Zeile an        
		return s; 														//Der Wert(ist nun eine  Zeichenkette) wird als s zurückgegeben
		}
}